...
each service owns its manifests (`deploy/`) in its own repository and deploys
by moving git refs that ArgoCD tracks (`deploy/dev`, `deploy/qa`, `main`); a
PR to the service `main` happens only after dev and qa. See
